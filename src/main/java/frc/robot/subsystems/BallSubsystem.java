...
public class BallSubsystem extends SubsystemBase {
        ...
        public BallSubsystem() {
                ...
                intakeDeployMotor.configure(intakeDeployConfig, ResetMode.kResetSafeParameters,
                                PersistMode.kPersistParameters);
                intakeRollingMotor.configure(intakeRollingConfig, ResetMode.kResetSafeParameters,
                                PersistMode.kPersistParameters);
                hopperDeployMotor.configure(hopperDeployConfig, ResetMode.kResetSafeParameters,
                                PersistMode.kPersistParameters);
                feederMotorLeft.configure(feederConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);
                feederMotorRight.configure(feederConfig, ResetMode.kResetSafeParameters,
                                PersistMode.kPersistParameters);
                shooterLeftLeader.getConfigurator().apply(shooterConfig);
                shooterRightLeader.getConfigurator().apply(shooterConfig);
                shooterLeftFollower
                                .setControl(new Follower(shooterLeftLeader.getDeviceID(), MotorAlignmentValue.Aligned));
                shooterRightFollower.setControl(
                                new Follower(shooterRightLeader.getDeviceID(), MotorAlignmentValue.Aligned));
        ...
        public void deployIntake() {
                intakeDeployMotor.set(SmartDashboard.getNumber("Intake deploy speed when deploying intake",
                                BallConstants.DEFAULT_INTAKE_DEPLOY_SPEED));
                intakeRollingMotor
                                .set(SmartDashboard.getNumber("Roller speed when intaking",
                                                BallConstants.DEFAULT_INTAKE_ROLLER_SPEED));
        }
        ...
        public void shootLeft() {
                feederMotorLeft
                                .set(SmartDashboard.getNumber("Feeder speed when feeding balls",
                                                BallConstants.DEFAULT_FEEDER_SPEED));
                shooterLeftLeader.set(
                                SmartDashboard.getNumber("Shooter speed when shooting balls",
                                                BallConstants.DEFAULT_SHOOTER_SPEED));
        }
