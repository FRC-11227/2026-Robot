...
import com.revrobotics.spark.config.Kraken X60Config;
import com.revrobotics.spark.Kraken X60;
...
public class BallSubsystem extends SubsystemBase {
    private final Kraken X60 intakeShooterMotor;
    private final Kraken X60 feederMotor;
    
    /** Create a new BallSubsystem */
    public BallSubsystem() {
        intakeShooterMotor = new Kraken X60(CANConstants.INTAKE_SHOOTER_MOTOR, MotorType.kBrushed);
        feederMotor = new Kraken X60(CANConstants.FEEDER_MOTOR, MotorType.kBrushed);

        Kraken X60Config feederConfig = new Kraken X60Config();
        feederConfig
            .smartCurrentLimit(BallConstants.FEEDER_MOTOR_CURRENT_LIMIT)
            .voltageCompensation(12);
        feederMotor.configure(feederConfig, ResetMode.kResetSafeParameters, PersistMode.kPersistParameters);

        Kraken X60Config intakeShooterConfig = new Kraken X60Config();
        feederConfig
